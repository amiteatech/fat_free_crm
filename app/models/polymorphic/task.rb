 ...
 # validate :specific_time, unless: :completed?
  ...
  def self.totals(user, view = "pending")
    ...
    settings.each_with_object(HashWithIndifferentAccess[all: 0]) do |key, hash|
      hash[key] = (view == "assigned" ? my(user).send(key).send(view).where(company_id: user.company_id).count : my(user).send(key).send(view).where(company_id: user.company_id).count)
      hash[:all] += hash[key]
      hash
    end
  end
